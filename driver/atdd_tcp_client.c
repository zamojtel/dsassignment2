...
static int hmac_tag_prepare(struct atdd_tcp_client *atdd_c)
{
  ...
  shash->flags = 0x0;
  return crypto_shash_digest(shash, atdd_c->send_buf + sizeof(u64),
    atdd_c->send_buf_pos - sizeof(u64), atdd_c->send_buf + atdd_c->send_buf_pos);
}
...
int prepare_cmd_content(struct atdd_tcp_client *atdd_c, struct request *rq, uint64_t ident)
{
  ...
  uint64_t temp;
  uint32_t temp32;
  bool write = rq_data_dir(rq) == 1;

  atdd_c->send_buf_pos = sizeof(u64); // will update final len at the end
 
  /* Client command */
  memset(atdd_c->send_buf + atdd_c->send_buf_pos, 0, sizeof(u32));
  atdd_c->send_buf_pos += sizeof(u32);

  temp = cpu_to_be64(ident);
  memcpy(atdd_c->send_buf + atdd_c->send_buf_pos, &temp, sizeof(temp));
  atdd_c->send_buf_pos += sizeof(temp);

  temp = cpu_to_be64(blk_rq_pos(rq) >> 3);
  memcpy(atdd_c->send_buf + atdd_c->send_buf_pos, &temp, sizeof(temp));
  atdd_c->send_buf_pos += sizeof(temp);

  temp32 = cpu_to_be32(write ? 1 : 0);
  memcpy(atdd_c->send_buf + atdd_c->send_buf_pos, &temp32, sizeof(temp32));
  atdd_c->send_buf_pos += sizeof(temp32);
  ...
  /* set size in header */
  temp = cpu_to_be64(atdd_c->send_buf_pos - sizeof(u64));
  memcpy(atdd_c->send_buf, &temp, sizeof(temp));
  return 0;
}
...
static int parse_status_code(enum status_code *code, uint32_t byte)
{
  switch (byte) {
      case 0:
        *code = OK;
...
static int progress_tcp_data(struct atdd_tcp_client *atdd_c)
{
  ...
  limit = be64_to_cpu(*(uint64_t*) (atdd_c->parse_buf)) + sizeof(u64);
  if (limit > MAX_RESPONSE_SIZE) {
    printk(KERN_WARNING "Invalid size %d", limit);
    return -EINVAL;
  }
  ...
  res = parse_status_code(&atdd_c->parse_state.st, *((uint32_t*) (atdd_c->parse_buf + pos)));
  pos += 4;
  if (res < 0) {
  ...
  atdd_c->parse_state.rq_ident = be64_to_cpu(*((uint64_t *) (atdd_c->parse_buf + pos)));
  pos += 8;

  op_type = be32_to_cpu(*((uint32_t *) (atdd_c->parse_buf + pos)));
  pos += 4;
...
int validate_cmd_parse_hmac(struct atdd_tcp_client *atdd_c)
{
  ...
  shash->tfm = atdd_c->hmac_alg;
  shash->flags = 0x0;
  if ((err = crypto_shash_digest(shash, atdd_c->parse_buf + sizeof(u64),
    atdd_c->parse_buf_pos - HMAC_TAG_SIZE - sizeof(u64), hmac_signature)))
    return err;
...
int atdd_fetch_command(struct atdd_tcp_client *atdd_c, struct atdd_cmd_return *cmd_r)
{
  ...
  cmd_r->data = atdd_c->parse_buf + ATDD_READ_RESPONSE_HEADER_SIZE;
